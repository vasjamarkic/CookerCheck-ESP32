...
bool stevec_start = 0;
float t_REF, t;                                   /*t_REF --> REFERENCE temperature measured at start-up, t --> periodically taken temperature */
float dT = 1.0;                                   /*prag zaznavanje kuhanja (med 1 in 3°C) */
...
void setup() {
  // put your setup code here, to run once:
    ...
    for(int k=0;k<5;k++) {
    toggleLED(BLUE);        // utripanje modre led 5x
    delay(400);
    }
    WiFi.mode(WIFI_STA);
...
void loop() {
  //Izvedba referenčne meritve - samo 1x: (s tipko EN znova postavimo T_REF)
  if (stevec_start == 0) {
